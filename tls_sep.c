...
int tls_sep_fromchild = -1;
int tls_sep_tochild = -1;
br_ssl_engine_context *tls_sep_eng;

int tls_sep_getcert(br_x509_certificate *chain, size_t *chain_len, char *key_type, const char *dir, const char *name) {
    ...
    /* write filename */
    if (pipe_writefn(tls_sep_tochild, dir, name) == -1) goto cleanup;
...
size_t tls_sep_mulgen(unsigned char *R, const unsigned char *x, size_t xlen, int curve) {
    ...
    log_t1("tls_sep_mulgen begin");

    /* finish certs */
    if (pipe_write(tls_sep_tochild, 0, 0) == -1) goto fail;

    /* write curve */
    if (pipe_write(tls_sep_tochild, &curve_id, sizeof curve_id) == -1) goto fail;

    /* read point */
    Glen = 133;
    if (pipe_readmax(tls_sep_fromchild, R, &Glen) == -1) goto fail;
    return Glen;

fail:
    log_e1("tls_sep_mulgen failed");
    return 0;
}

size_t tls_sep_dosign(const br_ssl_server_policy_class **pctx, unsigned int algo_id, unsigned char *data, size_t len, size_t max) {

    unsigned char hash_id = algo_id & 0xff;
    (void) pctx;

    log_t4("tls_sep_dosign begin: algo_id=", lognum(algo_id), ", len=", lognum(len));

    /* write hash_id */
    if (pipe_write(tls_sep_tochild, &hash_id, sizeof hash_id) == -1) goto fail;

    /* write max */
    if (pipe_write(tls_sep_tochild, &max, sizeof max) == -1) goto fail;

    /* write data */
    if (pipe_write(tls_sep_tochild, data, len) == -1) goto fail;

    /* read the signature */
    if (pipe_readmax(tls_sep_fromchild, data, &max) == -1) goto fail;

    log_t1("tls_sep_dosign success");
    return max;

fail:
    log_e1("tls_sep_dosign failed");
    return 0;
}

uint32_t tls_sep_mul(unsigned char *G, size_t Glen, const unsigned char *x, size_t xlen, int curve) {
    ...
    /* write pk */
    if (pipe_write(tls_sep_tochild, G, Glen) == -1) goto fail;

    /* write tls version, client_random, server_random */
    /* XXX is better place ?? */
    if (pipe_write(tls_sep_tochild, &tls_sep_eng->session.version, sizeof tls_sep_eng->session.version) == -1) goto fail;
    if (pipe_write(tls_sep_tochild, &tls_sep_eng->session.cipher_suite, sizeof tls_sep_eng->session.cipher_suite) == -1) goto fail;
    if (pipe_write(tls_sep_tochild, tls_sep_eng->client_random, sizeof tls_sep_eng->client_random) == -1) goto fail;
    if (pipe_write(tls_sep_tochild, tls_sep_eng->server_random, sizeof tls_sep_eng->server_random) == -1) goto fail;

    log_t1("tls_sep_mul success");
    return 1;

fail:
    log_e1("tls_sep_mul failed");
    return 0;
}

void tls_sep_prf(void *dst, size_t len, const void *secret, size_t secret_len, const char *label, size_t seed_num, const br_tls_prf_seed_chunk *seed) {

    unsigned char ch = tls_sep_PRF;
    (void) secret;
    (void) secret_len;
    ...
    if (pipe_write(tls_sep_tochild, &ch, sizeof ch) == -1) goto randomoutput;
    if (pipe_write(tls_sep_tochild, label, strlen(label) + 1) == -1) goto randomoutput;
    if (pipe_write(tls_sep_tochild, &seed->len, sizeof seed->len) == -1) goto randomoutput;
    if (pipe_write(tls_sep_tochild, seed->data, seed->len) == -1) goto randomoutput;
    if (pipe_readall(tls_sep_fromchild, dst, len) == -1) goto randomoutput;
    log_t3("tls_sep_prf(", label, ") finished");
    return;

randomoutput:
    log_t3("tls_sep_prf(", label, ") finished with random output");
    randombytes(dst, len);
}
...
static void in_chapol_init(const br_sslrec_in_chapol_class **cc, br_chacha20_run ichacha, br_poly1305_run ipoly, const void *key, const void *iv) {
    ...
    ctx->vtable.in = &tls_sep_chapol_in_vtable;
}
static void in_gcm_init(br_sslrec_gcm_context *cc, const br_block_ctr_class *bc_impl, const void *key, size_t key_len, br_ghash gh_impl, const void *iv) {
    ...
    cc->vtable.in = &tls_sep_gcm_in_vtable;
}
...
in_cbc_init(br_sslrec_in_cbc_context *cc,
    ...
    const void *iv)
{
    cc->vtable = &tls_sep_cbc_in_vtable;
    cc->seq = 0;
...
static unsigned char *decrypt(const br_sslrec_in_class **cc, int record_type, unsigned version, void *data, size_t *data_len) {
    unsigned char ch = tls_sep_DECRYPT;
    char offset = 0;
    (void) cc;
    log_t1("decrypt begin");
    if (pipe_write(tls_sep_tochild, &ch, sizeof ch) == -1) goto fail;
    if (pipe_write(tls_sep_tochild, &record_type, sizeof record_type) == -1) goto fail;
    if (pipe_write(tls_sep_tochild, &version, sizeof version) == -1) goto fail;
    if (pipe_write(tls_sep_tochild, data, *data_len) == -1) goto fail;
    if (pipe_readall(tls_sep_fromchild, &offset, sizeof offset) == -1) goto fail;
    if (pipe_readmax(tls_sep_fromchild, data + offset, data_len) == -1) goto fail;
...
const br_sslrec_in_chapol_class tls_sep_chapol_in_vtable = {
    {
...
const br_sslrec_in_gcm_class tls_sep_gcm_in_vtable = {
    {
...
const br_sslrec_in_cbc_class tls_sep_cbc_in_vtable = {
    {
...
static unsigned char *encrypt(const br_sslrec_out_class **cc, int record_type, unsigned version, void *data, size_t *data_len) {
    unsigned char ch = tls_sep_ENCRYPT;
    char offset;
    (void) cc;
    if (pipe_write(tls_sep_tochild, &ch, sizeof ch) == -1) goto cleanup;
    if (pipe_write(tls_sep_tochild, &record_type, sizeof record_type) == -1) goto cleanup;
    if (pipe_write(tls_sep_tochild, &version, sizeof version) == -1) goto cleanup;
    if (pipe_write(tls_sep_tochild, data, *data_len) == -1) goto cleanup;
    /* max overhead for gcm 29 */
    *data_len += 29;
    /* XXX */
    *data_len += 100;
    if (pipe_readall(tls_sep_fromchild, &offset, sizeof offset) == -1) goto cleanup;
    if (pipe_readmax(tls_sep_fromchild, data + offset, data_len) == -1) goto cleanup;
    return data + offset;
cleanup:
...
static void out_chapol_init(const br_sslrec_out_class **cc, br_chacha20_run ichacha, br_poly1305_run ipoly, const void *key, const void *iv) {
    ...
    ctx->vtable.out = &tls_sep_chapol_out_vtable;
}
...
{
    (void) bc_impl; (void) key; (void) key_len; (void) gh_impl; (void) iv;
    cc->vtable.out = &tls_sep_gcm_out_vtable;
}
...
out_cbc_init(br_sslrec_out_cbc_context *cc,
    ...
    log_d1("out_cbc_init");
    cc->vtable = &tls_sep_cbc_out_vtable;
    cc->seq = 0;
    bc_impl->init(&cc->bc.vtable, bc_key, bc_key_len);
    br_hmac_key_init(&cc->mac, dig_impl, mac_key, mac_key_len);
    cc->mac_len = mac_out_len;
...
const br_sslrec_out_chapol_class tls_sep_chapol_out_vtable = {
    {
        sizeof(br_sslrec_chapol_context),
...
const br_sslrec_out_gcm_class tls_sep_gcm_out_vtable = {
    {
        sizeof(br_sslrec_gcm_context),
...
const br_sslrec_out_cbc_class tls_sep_cbc_out_vtable = {
    {
