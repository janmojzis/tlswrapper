...
size_t tls_pipe_mulgen(unsigned char *R, const unsigned char *x, size_t xlen,
    ...
    /* write curve */
    if (pipe_write(tls_pipe_tochild, &curve_id, sizeof curve_id) == -1)
        goto fail;
...
uint32_t tls_pipe_mul(unsigned char *G, size_t Glen, const unsigned char *x,
    ...
    if (pipe_write(tls_pipe_tochild, &tls_pipe_eng->session.session_id,
                   sizeof tls_pipe_eng->session.session_id) == -1)
        goto fail;
    if (pipe_write(tls_pipe_tochild, &tls_pipe_eng->session.version,
                   sizeof tls_pipe_eng->session.version) == -1)
        goto fail;
    if (pipe_write(tls_pipe_tochild, &tls_pipe_eng->session.cipher_suite,
                   sizeof tls_pipe_eng->session.cipher_suite) == -1)
        goto fail;
    if (pipe_write(tls_pipe_tochild, tls_pipe_eng->client_random,
                   sizeof tls_pipe_eng->client_random) == -1)
        goto fail;
    if (pipe_write(tls_pipe_tochild, tls_pipe_eng->server_random,
                   sizeof tls_pipe_eng->server_random) == -1)
        goto fail;
...
void tls_pipe_prf(void *dst, size_t len, const void *secret, size_t secret_len,
    ...
    if (pipe_write(tls_pipe_tochild, &seed->len, sizeof seed->len) == -1)
        goto randomoutput;
    if (pipe_write(tls_pipe_tochild, seed->data, seed->len) == -1)
        goto randomoutput;
...
static int chapol_check_length(const br_sslrec_in_class *const *cc,
                               size_t rlen) {
...
static void in_chapol_init(const br_sslrec_in_chapol_class **cc,
...
static void in_gcm_init(br_sslrec_gcm_context *cc,
...
static void in_cbc_init(br_sslrec_in_cbc_context *cc,
                        const br_block_cbcdec_class *bc_impl,
                        const void *bc_key, size_t bc_key_len,
                        const br_hash_class *dig_impl, const void *mac_key,
                        size_t mac_key_len, size_t mac_out_len,
                        const void *iv) {
...
static void gcm_max_plaintext(const br_sslrec_out_class *const *cc,
                              size_t *start, size_t *end) {
    br_sslrec_out_gcm_vtable.inner.max_plaintext(cc, start, end);
}

static void cbc_max_plaintext(const br_sslrec_out_class *const *cc,
                              size_t *start, size_t *end) {
...
static unsigned char *encrypt(const br_sslrec_out_class **cc, int record_type,
    ...
    if (pipe_write(tls_pipe_tochild, &record_type, sizeof record_type) == -1)
        goto cleanup;
    if (pipe_write(tls_pipe_tochild, &version, sizeof version) == -1)
        goto cleanup;
...
static void out_gcm_init(br_sslrec_gcm_context *cc,
                         const br_block_ctr_class *bc_impl, const void *key,
                         size_t key_len, br_ghash gh_impl, const void *iv) {
    (void) bc_impl;
    (void) key;
...
static void out_cbc_init(br_sslrec_out_cbc_context *cc,
