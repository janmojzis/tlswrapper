...
extern void br_ssl_engine_switch_chapol_out(br_ssl_engine_context *cc, int is_client, int prf_id);
extern void br_ssl_engine_switch_gcm_in(br_ssl_engine_context *cc, int is_client, int prf_id, const br_block_ctr_class *bc_impl, size_t cipher_key_len);
extern void br_ssl_engine_switch_gcm_out(br_ssl_engine_context *cc, int is_client, int prf_id, const br_block_ctr_class *bc_impl, size_t cipher_key_len);
extern void br_ssl_engine_compute_master(br_ssl_engine_context *cc, int prf_id, const void *pms, size_t pms_len);
extern void br_ssl_engine_switch_cbc_in(br_ssl_engine_context *cc, int is_client, int prf_id, int mac_id, const br_block_cbcdec_class *bc_impl, size_t cipher_key_len);
...
static size_t sign(struct tls_pem *pem, unsigned char *key, br_ssl_server_context *cc, unsigned char hash_id, unsigned char *data, size_t hv_len, size_t len) {
    ...
    if (keycrt.key_type == BR_KEYTYPE_EC) {
        br_ssl_server_init_full_ec(cc, 0, 0, 0, br_skey_decoder_get_ec(&keycrt.keydc));
        vtable = &cc->chain_handler.single_ec.vtable;
    }
    if (keycrt.key_type == BR_KEYTYPE_RSA) {
        br_ssl_server_init_full_rsa(cc, 0, 0, br_skey_decoder_get_rsa(&keycrt.keydc));
...
static void prf(struct tls_context *ctx, void *dst, size_t len, int prf_id, const char *label, unsigned char *seed_data, size_t seed_data_len) {
...
void tls_sep_child(struct tls_context *ctx) {
    ...
    */
    ...
    /* KEYJAIL starts here */
    if (jail(ctx->empty_dir) == -1) goto cleanup;

    /* scalar multiplication - keygen */
    if (pipe_readall(0, &curve_id, sizeof(curve_id)) == -1) goto cleanup;
    randombytes(sk, sizeof sk);
    if (tls_crypto_scalarmult_base((unsigned int) curve_id, pk, &pklen, sk) == -1) goto cleanup;
    ...
    if (pipe_readall(0, cc->eng.server_random, sizeof cc->eng.server_random) == -1) goto cleanup;
    if (tls_crypto_scalarmult((unsigned int) curve_id, pk, &pklen, sk) == -1) goto cleanup;

    randombytes(sk, sizeof sk); /* remove secret scalar */
    
    log_t2("SUITE: ", tls_cipher_str(cc->eng.session.cipher_suite));
    prf_id = br_sha1_ID;
    switch(tls_cipher_get(cc->eng.session.cipher_suite)) {
        ...
        case tls_cipher_AES_256_GCM_SHA384:
            prf_id = br_sha384_ID;
            br_ssl_engine_compute_master(&cc->eng, prf_id, pk, pklen);
            br_ssl_engine_switch_gcm_in(&cc->eng, 0, prf_id, cc->eng.iaes_ctr, 32);
            br_ssl_engine_switch_gcm_out(&cc->eng, 0, prf_id, cc->eng.iaes_ctr, 32);
            break;
        case tls_cipher_AES_128_GCM_SHA256:
            prf_id = br_sha256_ID;
            br_ssl_engine_compute_master(&cc->eng, prf_id, pk, pklen);
            br_ssl_engine_switch_gcm_in(&cc->eng, 0, prf_id, cc->eng.iaes_ctr, 16);
            br_ssl_engine_switch_gcm_out(&cc->eng, 0, prf_id, cc->eng.iaes_ctr, 16);
            break;
        case tls_cipher_AES_256_CBC_SHA384:
            prf_id = br_sha384_ID;
            br_ssl_engine_compute_master(&cc->eng, prf_id, pk, pklen);
            ...
            break;
        case tls_cipher_AES_256_CBC_SHA:
            br_ssl_engine_compute_master(&cc->eng, prf_id, pk, pklen);
            br_ssl_engine_switch_cbc_in(&cc->eng, 0, prf_id, prf_id, cc->eng.iaes_cbcdec, 32);
            br_ssl_engine_switch_cbc_out(&cc->eng, 0, prf_id, prf_id, cc->eng.iaes_cbcenc, 32);
            break;
        case tls_cipher_AES_128_CBC_SHA256:
            prf_id = br_sha256_ID;
            br_ssl_engine_compute_master(&cc->eng, prf_id, pk, pklen);
            ...
            break;
        case tls_cipher_AES_128_CBC_SHA:
            br_ssl_engine_compute_master(&cc->eng, prf_id, pk, pklen);
            br_ssl_engine_switch_cbc_in(&cc->eng, 0, prf_id, prf_id, cc->eng.iaes_cbcdec, 16);
            br_ssl_engine_switch_cbc_out(&cc->eng, 0, prf_id, prf_id, cc->eng.iaes_cbcenc, 16);
            break;
        ...
        switch (ch) {
            case tls_sep_PRF:
                {
                    ...
                    if (pipe_readall(0, &seed_len, sizeof seed_len) == -1) goto cleanup;
            ...
            case tls_sep_DECRYPT:
                /* decrypt */
                {
                    ...
                    char offset;
                    void *ret;
                    if (pipe_readall(0, &record_type, sizeof record_type) == -1) goto cleanup;
                    if (pipe_readall(0, &version, sizeof version) == -1) goto cleanup;
                    data_len = sizeof data - 64;
                    if (pipe_readmax(0, data + 64, &data_len) == -1) goto cleanup;
                    ret = cc->eng.in.vtable->decrypt(&cc->eng.in.vtable, record_type, version, data + 64, &data_len);
                    if (!ret) {
                        data_len = 0;
                        offset = 0;
            ...
            case tls_sep_ENCRYPT:
                /* encrypt */
                {
                    ...
                    data_len = sizeof data - 64;
                    if (pipe_readmax(0, data + 64, &data_len) == -1) goto cleanup;
                    ret = cc->eng.out.vtable->encrypt(&cc->eng.out.vtable, record_type, version, data + 64, &data_len);
                    offset = (char)(long long)((unsigned long long)ret - (unsigned long long)data - 64);
                    if (pipe_write(1, &offset, sizeof offset) == -1) goto cleanup;
                    if (pipe_write(1, ret, data_len) == -1) goto cleanup;
                }
