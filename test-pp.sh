...
${CMD} -w tlswrappernojail -P1 -d "`pwd`/testcerts" sh -c 'exec >&9; printenv TCPREMOTEIP; cat' <data.in 9>&1 2>&1
...
(
  ...
  ${CMD} -P "${line}" -w tlswrappernojail -p1 -P2 -d "`pwd`/testcerts" sh -c 'exec cat >&9' <data.in 9>&1 2>&1 | od -c
  ${CMD} -P "${line}" -w tlswrappernojail -p1 -P1 -d "`pwd`/testcerts" sh -c 'exec >&9; printenv TCPREMOTEIP; cat' <data.in 9>&1 2>&1
done
